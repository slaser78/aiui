...
export default function SetSubnets (props) {
    const urlValue = useContext(UrlContext);

    const handleAction = async () => {
        props.setAuditVuln(false);
        await axios.get(urlValue.urlValue + '/getAuditResults', options)
            .then(() => {
            }, (error) => {
                console.log(error);
            });
    ...
    return (
        <Dialog open={props.open}>
            <DialogTitle>Get Audit Results</DialogTitle>
            <DialogContent>
