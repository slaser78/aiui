...
export default function DeleteMit (props) {

    const urlValue = useContext(UrlContext);

    const handleAction = async () => {
        props.setDeleteMit(false);
        await axios.get(urlValue.urlValue + '/deleteMit', options)
            .then(() => {
            }, (error) => {
                console.log(error);
            });
    ...
    return (
        <Dialog open={props.open}>
            <DialogTitle>Delete Empty Mitigations</DialogTitle>
            <DialogContent>
                    <div className="row">
