...
export default function GetVulnUpdate (props) {
    const options = {
        headers: {
    ...
    const handleAction = async () => {
        props.setGetVulnUpdate(false);
        await axios.get(urlValue.urlValue + '/getVulnUpdate', options)
            .then(() => {
            }, (error) => {
                console.log(error);
            });
    ...
    return (
        <Dialog open={props.open}>
            <DialogTitle>Get Vuln Update</DialogTitle>
            <DialogContent>
